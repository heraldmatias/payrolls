...
//        exit;
        if ($source === 1) {
            ...
            try {
                $this->em->beginTransaction();
                if (null === $obj) {
                    $this->em->getConnection()->executeUpdate($sqli, array(
                        'persona' => $persona
                    ));
                }
    ...
    public function registraPersona($form, $nombres) {
        $_nombres = array();
        foreach ($nombres as $nombre) {
            $_nombres[] = preg_replace("('|´|,|\.| )", "", $nombre);
        }
        $nombres = "('" . implode("','", $_nombres) . "')";
        $sqli = 'SELECT fn_asocia_per_nuevo(:apepat, :apemat, :nombre, :desnombre, :dni) AS codigo';
        $sql = "UPDATE personal_digitado 
                SET codi_empl_per_persona = :persona
                WHERE regexp_replace(nomb_cort_per,'\W+', '','g') IN " . $nombres;
        $form['desnombre'] = sprintf('%s %s %s', $form['apepat'], $form['apemat'], $form['nombres']);
        try {
            $this->em->beginTransaction();
            $stpr = $this->em->getConnection()->prepare($sqli);
            $stpr->bindValue(':apepat', $form['apepat']);
            $stpr->bindValue(':apemat', $form['apemat']);
            $stpr->bindValue(':nombre', $form['nombres']);
            $stpr->bindValue(':desnombre', $form['desnombre']);
            $stpr->bindValue(':dni', $form['dni']);
            $stpr->execute();
            $data = $stpr->fetch(\Doctrine\ORM\Query::HYDRATE_ARRAY);
